...
- 首页和呼吸页使用横向手势切换主功能：首页右滑进入呼吸页、左滑进入设置页；呼吸页右滑返回首页、左滑进入设置页。进入设置页时通过 `from=home` / `from=zen` 记录来源；设置页点击“保存返回”时，应返回对应的首页普通状态或呼吸页状态。
...
- 首页底部普通状态只保留设置 `···` 和环境音 `♪` 两个入口；视频切换只通过上下滑动手势触发，横向手势用于首页 / 呼吸页 / 设置页之间切换。
...
- 首页 / 呼吸页横向手势已调整：首页右滑进入呼吸页、左滑进入设置页；呼吸页右滑返回首页、左滑进入设置页。设置页新增“保存返回”，并根据 `from=home` / `from=zen` 返回对应状态。
...
- 上述“去掉引号按钮 / 设置页紧凑布局 / 天气真机兜底 / 手势与播放序列 / 文档更新”是在 `f0a101d` 之后的新本地改动；截至本条记录写入时尚未提交和推送。
