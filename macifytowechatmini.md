...
| Pixabay | 100 | 42 | 0 | 42 | 2026-06-05 删除 `静海红日`、`岩瀑白浪`、`瑞士雾海` 后保留 42 条；当前公开池共 43 条，包含 19 条 Flutter 首包内置和 24 条远端备选；`木舟渔人`、`稻田风机`、三条短循环试验视频及其原片备份均已移入本地废弃视频目录，COS 远端对象已删除 |
...
- COS 视频 URL 必须带 `premiumFreeAerialSourceVersion` query，例如 `?v=<contentVersion>`，用于绕开同路径 MP4 覆盖后的微信视频组件 / 网络缓存；小程序本地文件缓存也按该版本号隔离。
...
- 公开版小程序优先播放体验：播放队列抽到视频时先查手机本地缓存，命中则把 `<video>` 的 `src` 指向本地文件；未命中缓存时直接播放 COS URL，不显示“正在缓存视频”等待文案，不为了省流量卡住首播。
- 微信官方文件系统文档说明，本地缓存文件和本地用户文件合计上限为 `200MB`，`wx.env.USER_DATA_PATH` 自定义目录也属于本地用户文件。因此小程序不能做 `1GB` 或完整 48 条视频的持久缓存；当前 48 条本地正式 MP4 仍远超小程序可长期缓存上限，全量缓存只能在 Flutter App 侧实现。
- 本地视频缓存使用 `wx.env.USER_DATA_PATH/macify-video-cache/` 和 `video-file-cache-v2` 索引。缓存按当前内容版本隔离，内容版本变化时清理旧版本文件。
- 小程序缓存只保留有限的历史本地文件作为复看优化；不得承诺全量视频缓存，不做 `4` 条本地 + `1` 条新下载这类会牺牲新鲜感或首播速度的策略。
...
- 本地缓存只作为省 COS 流量和加速复看的优化；缓存命中优先播放本地，缓存未命中直接播放远端 COS。
...
- 呼吸提示音只在前台播放包内素材；颂钵音使用 `miniprogram/assets/breath.mp3`，人声提示使用 `miniprogram/assets/voice-inhale.m4a`、`voice-exhale.m4a`、`voice-hold.m4a`，自定义练习开始 / 完成分别使用 `voice-practice-start.m4a` / `voice-practice-complete.m4a`。锁屏 / 后台不播放呼吸提示音，也不需要上传远程 `breath.mp3` 或人声素材。
- 设置页“视频背景音”当前有两种模式：
...
- 自定义混音设置保存在 `settings.customAmbientMix`，模式保存在 `settings.ambientAudioMode`。每条混音记录是 `{ trackId, volume }`，`volume` 范围 `0-1`，语义是混音权重，不是最终播放器线性音量。
...
- 自定义混音里每个音轨选中后高亮，右侧显示音量滑杆和实时百分比读数；默认音量为 `0`。用户点击“开始试听”后可边调音边试听，例如拖到 `森林54% + 鸟鸣15%` 时，UI、存储和内容 JSON 仍保持 `0.54 + 0.15`，播放前按最高非零权重归一化为 `森林100% + 鸟鸣约27.8%`；`40% / 30% / 10%` 会实际播放为 `100% / 75% / 25%`。这样百分比仍表达相对混音比例，但不会把整体声音一起压小。
- 首页 / 呼吸页 `♪` 打开自定义混音时，如果用户尚未把任何音轨音量调到非零，应提示去设置里调大混音音量。
- 环境音使用 `wx.createInnerAudioContext({ useWebAudioImplement: true })`；循环播放不依赖硬 `loop`，而是在接近结尾时启动第二个同源音频实例，并用约 `6s` 交叉淡入淡出，避免循环断点。
...
- 音量必须克制，但不能把混音权重直接当最终系统音量压低。已用 `ffmpeg volumedetect` 检查原始音频响度，并转码出上线用 128kbps MP3；代码里的 `volume` 表示相对配比，播放前按每组最高非零权重归一化，单轨非零实际以满音量播放，多轨保留相对大小。
...
当前 48 条公开视频的环境音映射统计（按音轨计数；多轨视频会在多个音轨里各计 1 次）：

```text
风声 wind-in-trees.mp3: 36 条
海浪海鸥 gentle-ocean-waves-birdsong-and-gull.mp3: 12 条
水下 underwater-ambience.mp3: 9 条
天空 mountain-sky-ambience.mp3: 8 条
海浪 ocean-soft-waves.mp3: 8 条
瀑布 waterfall.mp3: 7 条
溪流 river-stream.mp3: 6 条
森林 forest-ambience.mp3: 6 条
山林鸟鸣 forest-wind-and-birds.mp3: 5 条
...
- 呼吸页底部保留 `触感`、`颂钵音`、`人声提示`、`自定义练习` 四个文字入口；`颂钵音` 和 `人声提示` 是独立开关，可以同时开启。点击自定义练习或在呼吸页右滑后，花朵先隐藏，中央显示 `3 / 2 / 1` 倒计时，并展示练习说明。
- 设置页不再提供“呼吸提示音”二选一；`颂钵音` 仍使用本地 `miniprogram/assets/breath.mp3` 循环播放；`人声提示` 使用本地 `voice-inhale.m4a`、`voice-exhale.m4a`、`voice-hold.m4a`，在对应吸气 / 呼气 / 屏息 phase 开始时播放一次。自定义练习开始 / 完成时，如果人声提示开关开启，还会分别播放 `voice-practice-start.m4a` / `voice-practice-complete.m4a`。颂钵音和人声提示都按 `1.0` 播放，且小程序和 Flutter 均使用同一套双开关语义。
- 设置页的 `保留呼吸页设置` 只影响呼吸页底部的 `触感`、`颂钵音`、`人声提示` 三个开关，不影响首页 / 呼吸页环境音 `♪`。
- `保留呼吸页设置` 关闭时，真正退出后重新进入小程序会自动关闭 `zenHaptics`、`zenSound` 和 `zenVoiceCue`；打开时可以跨普通重新进入保留触感和呼吸提示音偏好。但锁屏、按 Home、从微信临时切到后台都要停止本次呼吸提示音，回前台默认无声。
- 微信 `App.onHide` 触发时，无论是锁屏、切后台还是真正退出，都停止 `zenSound`、`zenVoiceCue` 和当前呼吸提示音，避免后台或重新打开时突然发声。触感是否跨普通重新进入保留仍按 `rememberZenCues` 设置执行。
...
- 呼吸触感只在用户打开 `触感` 时触发。吸气触感随吸气时长缩放；有屏息阶段时，在屏息切换到下一阶段的节点播放轻触感提示；未开启触感或对应屏息为 `0` 时不得振动。2026-06-28 Flutter iOS App 已补齐 `hold-after-inhale -> exhale` 切换时的双轻触感，和小程序屏息到呼气提示保持一致。同日小程序和 Flutter App 已新增自定义练习完成触感：完成自定义练习后播放两次强烈长震，且只在 `触感` 开启时触发。Flutter App 呼吸页字体 / 间距也必须继续贴近小程序：用轻字重、低透明度、弱阴影和低存在感底部控件，不使用 Material 按钮默认粗重视觉；设置页呼吸节奏必须一行紧凑展示，不允许依赖横向滑动才能看完整公式。2026-06-30 App 已把呼吸节奏公式拆为标签行和输入行，输入行数字格改为外层固定小格 + 内层 collapsed `TextField`，确保数字、箭头、`秒 ×`、`组` 在同一输入行内居中对齐；设置页顶部、卡片、开关行和底部保存栏也要保持紧凑，尽量同屏展示更多设置。
...
2026-06-04 按用户要求删除 `麦田收割`、`墨云入水`、`营火煮茶`、`碧岛航岸`、`彩墨云生`、`山田星田`（用户写“山间星田”，当前内容源命中此条）、`水母成群`、`蓝岸斜浪`、`阿尔卑斯山径`、`蓝海航岸`、`雪河气泡`、`蓝幕水母`，并删除仅特殊匹配的 `收割机` 音频。共享内容源已从 60 条公开视频 / 13 条环境音调整为 48 条公开视频 / 12 条环境音；`contentVersion` bump 到 `premium-free-aerial-1080p-cos-20260604-cleaned-48`；小程序生成数据、Flutter `assets/content`、Flutter bundled 清单和手工筛选表已同步。本地正式 MP4 和 `tractor-harvesting.mp3` 已移入 `.gitignore` 覆盖的废弃目录。COS 已物理删除 12 个 `macify-premium/videos/*.mp4` 和 `macify-audio/tractor-harvesting.mp3`；`content-dist/*.json` 已上传到 `macify-premium/` 和 `macify-premium/content/`；旧 `macify-premium/manifest.json` / `manifest.csv` 也已覆盖为 48 条。远端校验显示删除对象均不再返回 200，`content/content-manifest.json` 与本地字节一致。本次密钥只作为命令进程和临时 COSCLI 配置使用，临时配置已删除，未写入仓库。

2026-06-05 用户确认 3 秒短循环效果不好，正式删除 `静海红日`、`岩瀑白浪`、`瑞士雾海`。共享内容源从 46 条公开视频 / 46 条显式混音缩减为 43 条公开视频 / 43 条显式混音；`contentVersion` bump 到 `premium-free-aerial-1080p-cos-20260605-cleaned-43`；小程序生成数据、Flutter `assets/content` 和手工筛选表已同步。此前已从公开池移除但本地 / COS 未清理的 `木舟渔人`、`稻田风机` 也一并完成收尾：5 个本地正式 MP4 已移入 `local-miniprogram-premium-aerial/abandoned/20260604-video-audio-cleanup/videos/`，三条短循环试验的原片备份也已移入同一废弃视频目录；5 个远端 `macify-premium/videos/*.mp4` 已物理删除并验证不再返回 `200`；`content-dist/*.json` 已重新上传到 `macify-premium/` 和 `macify-premium/content/`。
...
首页新增右下角 `♪` 环境音开关。默认无声音；每次退出、切后台或重新进入小程序都会关闭；首页和呼吸页之间切换时保持同一个环境音开关状态。用户主动点 `♪` 但当前视频无匹配音频时提示“当前视频暂无匹配音频”；如果是滑动切到无音源视频，则只临时淡出静音，不关闭用户开声意图，下一条有音源时自动恢复。环境音通过 `wx.createInnerAudioContext({ useWebAudioImplement: true })` 播放，循环时使用两个音频实例交叉淡入淡出；当前交叉时间已扩到约 `6s`，切换到不同环境音时也做淡入淡出。呼吸页呼吸提示音仍使用包内素材，与首页环境音互不共享状态。
...
- 自定义混音最多 5 个声音；每个声音默认音量 0，选中后高亮并出现音量滑杆和实时百分比读数。设置页提供“开始试听”，试听和首页播放都使用每个音轨独立 channel + 交叉淡入淡出循环；`森林54% + 鸟鸣15%` 这类数字表达预制混音权重，实际播放前会按组归一化为 `森林100% + 鸟鸣约27.8%`。
...
- 设置页天气配置改为紧凑布局：城市输入框 + 摄氏度 / 华氏度按钮放在“显示”区天气开关下方，只有天气开关打开时展示；默认城市改为 `北京`。2026-07-02 Flutter App 侧继续压缩显示区，`时间`、`天气`、`语录`、`视频信息` 改为一行四个等宽文字气泡，亮态表示开启、暗态表示关闭，不再占用四条 switch 行。同日 App 侧播放范围改为标题单独一行、6 个分类选项下一行铺满卡片宽度，并移除“当前内容库 43 条公开视频”；播放范围按钮复用显示区气泡的固定外壳，强制撑满 38px 高度，文字允许两行换行；背景音频改为“背景音频 + 视频自带音频 / 自定义混音”同一行；呼吸节奏外层标题去掉，只保留 `呼吸节奏- 默认` 和 `自定义呼吸练习`，节奏公式内部左对齐并强制占满卡片宽度；设置页标题区把 `安静的呼吸与风景` 放到 `© 呼吸Zen` 同一行右侧。2026-07-03 Flutter App 侧统一设置页标题行样式，`显示`、`播放范围`、`呼吸节奏- 默认`、`自定义呼吸练习`、`背景音频` 使用同一字体字号颜色。
- 背景视频播放范围改成一行 6 个直接铺开的高亮选项，减少设置页长度。
...
- 新增 `npm run mini:ambient:validate`，校验音轨、显式视频混音、音量范围、视频 ID、音轨 ID，并统计 published 视频的环境音覆盖情况。2026-06-04 当前结果：12 条音轨、48 条显式视频混音、48/48 条公开视频有环境音、0 条无音频。
...
- 时间 / 天气 / 语录 / 视频信息：App 侧使用一行四个文字气泡，亮态表示开启、暗态表示关闭
- 保留呼吸页设置，默认关闭；只影响触感和呼吸提示音，切后台再回来仍先静音
- 城市设置
- 摄氏/华氏
- 视频播放范围，App 侧标题单独一行，6 个大按钮下一行铺满卡片宽度，不再显示内容库数量说明
- 背景音频，App 侧使用一行文字标签 + 视频自带音频 / 自定义混音
- 视频源选择
  - 内置 1080P视频
  - 轻量 CDN
- 反向代理开关
...
- 标签显示 `吸气 / 呼气`
- 冥想页内新增 `触感` 开关，默认关闭
- 重新进入小程序时，默认（未开启“保留呼吸页设置”）会自动关闭触感和呼吸提示音，避免突然振动或响声
- 设置页新增 `保留呼吸页设置`，默认关闭；打开后会记住上次触感/呼吸提示音偏好，但切后台再回来仍先关闭呼吸提示音，避免自动恢复声音
- 触感 / 呼吸提示音控制放在冥想页最底部，不放在花朵和呼吸标签附近
- 控制不使用 `switch`，直接点击 `触感` / 当前模式下的 `颂钵音` 或 `人声提示` 文字切换状态
...
- 冥想页内新增呼吸提示音开关，默认关闭；按钮文案按设置页 `zenCueAudioMode` 显示为 `颂钵音` 或 `人声提示`
- 颂钵音方案：
...
- 人声提示方案（2026-06-01 新增小程序 + Flutter 同步版）：
  - 使用本地 `miniprogram/assets/voice-inhale.m4a`、`voice-exhale.m4a`、`voice-hold.m4a`
  - Flutter 使用同名素材 `flutter_app/assets/media/audio/voice-inhale.m4a`、`voice-exhale.m4a`、`voice-hold.m4a`
  - 三段素材由用户提供的 `吸气.m4a`、`呼气.m4a`、`屏息.m4a` 重新 loudnorm 到约 `-18 LUFS`，true peak 低于 `-3 dB`，播放音量设为 `1.0`
  - 设置页“呼吸提示音”选择 `人声提示` 后，不启动颂钵循环
  - 在每个吸气 / 呼气 / 屏息 phase 开始时播放一次对应录音，屏息前后共用 `voice-hold.m4a`
...
- 历史记录：旧版只缓存 `videoSource === 'lite'` 的最近 1 条 MP4，不缓存 Apple 官方 MOV；当前公开版已改为顶部硬性原则里的 `video-file-cache-v2` 多条本地缓存策略。
- 用户第一次进入时：
  - 如果找到上一次缓存的视频文件，继续播放这条本地文件。
  - 如果找不到缓存，则随机选择一条当前视频库的视频。
- 用户不点击换视频时：
  - 当前视频循环播放，不再自动切下一条。
  - 旧版如果当前视频是远程 MP4，会另行保存到 `wx.env.USER_DATA_PATH/macify-video-cache/`。
  - 当前新版优先体验：有缓存就播本地，没缓存就直接播远端 COS；App 侧承担更大的长期缓存目标。
- 用户点击右下角换视频或下拉刷新时：
  - 才主动随机选择新视频。
  - 当前新版新视频直接播放；小程序缓存只保留已有本地文件的复看优化。
- 如果本地文件被微信清理或不存在：
